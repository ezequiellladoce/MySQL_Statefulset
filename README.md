# k8s MySQL StatefulSet with Secrets

In this replo we will deploy a Mysql Stateful with secrets. A StatefulSet , the pods, can record data persistently in a persistent volume and can replicate data to the other pods in the same StatefullSet
...
- Deploy a simple StatefulSet MySql db
- Add a Headless service to provide a stable domain controller of the pods.
- Use  secrets to mount the passwords in the pods
...
- secret.yaml this YAML is used to mount the password as a secret
- statefulset.yaml this YAML file describes the Stateful set and associated the pods to the persisten volume
- service.yaml this YAML provides the Headless service

## Clone the repository 

```
git clone https://github.com/ezequiellladoce/MySQL_Statefulset.git
```
...
```
kubectl create namespace k8s-mysql

kubectl get namespace
```
### Create the secret
```
kubectl -f apply secret.yaml -n k8s-mysql
```
Check the secret 
```
